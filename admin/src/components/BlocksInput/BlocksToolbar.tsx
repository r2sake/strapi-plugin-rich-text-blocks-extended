...
const BlocksDropdown = () => {
  ...
  const handleSelect = (optionKey: unknown) => {
    ...
    const editorIsEmpty =
      editor.children.length === 1 && Editor.isEmpty(editor, editor.children[0] as never);
    ...
    const currentListEntry = Editor.above(editor, {
      match: (node) => !Editor.isEditor(node) && 'type' in node && node.type === 'list',
    });
    ...
    if (currentListEntry && ['list-ordered', 'list-unordered'].includes(optionKey)) {
      ...
      if (!Editor.isEditor(currentList) && isListNode(currentList)) {
        ...
        if ((currentList as ListNode).format !== format) {
          Transforms.setNodes(editor, { format } as Block<'list'>, { at: currentListPath });
        }
      }
      return;
    }
  ...
  React.useEffect(() => {
    if (editor.selection) {
      ...
      const anchorBlockKey = getKeys(blocks).find(
        (blockKey) => !Editor.isEditor(selectedNode) && blocks[blockKey].matchNode(selectedNode as never)
      );
...
const isListNode = (node: unknown): node is Block<'list'> => {
  return Node.isNode(node) && !Editor.isEditor(node) && 'type' in node && node.type === 'list';
};
...
const ListButton = ({ block, format, location = 'toolbar' }: ListButtonProps) => {
  ...
  const isListActive = () => {
    ...
    if (currentListEntry) {
      const [currentList] = currentListEntry;
      if (!Editor.isEditor(currentList) && isListNode(currentList) && (currentList as ListNode).format === format)
        return true;
    }
    return false;
  };
  ...
  const toggleList = (format: Block<'list'>['format']) => {
    ...
    if (!currentListEntry) {
      ...
      (blocks[`list-${format}`] as { handleConvert: (editor: Editor) => void }).handleConvert!(editor);
      return;
    }
  ...
  const isLinkActive = () => {
    ...
    const [match] = Array.from(
      Editor.nodes(editor, {
        ...
        match: (node) => SlateElement.isElement(node) && 'type' in node && node.type === 'link',
      })
    );
...
const BlocksToolbar = () => {
  ...
  const observedComponents: ObservedComponent[] = [
    ...Object.entries(modifiers).map(([name, modifier]) => {
      const Icon = (modifier as { icon: React.ComponentType<React.SVGProps<SVGSVGElement>> }).icon;
      const isActive = (modifier as { checkIsActive: (editor: Editor) => boolean }).checkIsActive(editor);
      const handleSelect = () => (modifier as { handleToggle: (editor: Editor) => void }).handleToggle(editor);
      ...
      return {
        toolbar: (
          <ToolbarButton
            key={name}
            name={name}
            icon={(modifier as { icon: React.ComponentType<React.SVGProps<SVGSVGElement>> }).icon}
            label={(modifier as { label: MessageDescriptor }).label}
            isActive={(modifier as { checkIsActive: (editor: Editor) => boolean }).checkIsActive(editor)}
    ...
    },
    {
      ...
      toolbar: (
        <Flex direction="row" gap={1}>
          ...
          <Toolbar.ToggleGroup type="single" asChild>
            <Flex gap={1}>
              <ListButton block={blocks['list-unordered']} format={"unordered" as never} location="toolbar" />
              <ListButton block={blocks['list-ordered']} format={"ordered" as never} location="toolbar" />
            </Flex>
          </Toolbar.ToggleGroup>
        </Flex>
      ),
      menu: (
        <>
          <Menu.Separator />
          <ListButton block={blocks['list-unordered']} format={"unordered" as never} location="menu" />
          <ListButton block={blocks['list-ordered']} format={"ordered" as never} location="menu" />
        </>
      ),
      key: 'block.list',
    },
